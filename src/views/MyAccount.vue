              ...
              <div role="tablist" id="accordion">
                <b-card no-body class="mb-3">
                  <b-card-header header-tag="header" class="p-1" role="tab">
                    <a>
                      <h4>
                        <strong>{{ $t('views.my_account.account_details') }}</strong>
                      </h4>
                    </a>
                  </b-card-header>
                  ...
                  <b-card-body style="position: relative;">
                    <div class="row mt-4">
                      <div class="col mb-4">
                        <h5 class="rmv-spacing" style="display:none">
                          <strong>{{ $t('views.my_account.my_address') }}</strong>
                        </h5>
                        <div class="address-container" style="display:none">
                          ...
                          <b-button
                            ...
                            v-clipboard:copy="userAccount.address"
                          >
                            <fa :icon="['fa', 'copy']" class="text-gray rmv-spacing"/>
                          </b-button>
                        </div>
                        <div class="balance-container mt-2">
                          <h5>
                            <strong>{{ $t('views.my_account.balance') }}</strong>
                          </h5>
                          <h6>
                            {{ $t('views.my_account.mainnet') }}
                            <strong>{{userBalance.isLoading ? 'loading' : userBalance.mainnetBalance + " LOOM"}}</strong>
                          </h6>
                          <div v-if="allowance && !gatewayBusy">
                            <small>
                              <fa icon="fa exclamation-triangle"/>
                              {{allowance}} LOOM out of mainnet balance awaiting transfer to plasmachain account
                            </small>
                            <b-btn
                              size="sm"
                            ...
                            >resume deposit</b-btn>
                          </div>
                          <h6>
                            {{ $t('views.my_account.plasmachain') }}
                            <strong>{{userBalance.isLoading ? 'loading' : userBalance.loomBalance + " LOOM"}}</strong>
                          </h6>
                        </div>
                      </div>
                    ...
                    <div class="row">
                      <div class="col-md-6">
                        <a ref="accordion2Toggle">
                          <h4>
                            <strong>{{ $t('views.my_account.deposit_withdraw') }}</strong>
                          </h4>
                        </a>
                      </div>
                    </div>
                        ...
                        <b-card no-body v-if="metamaskConnected">
                          <b-tabs card>
                            <b-tab
                              ...
                              v-if="!oracleEnabled"
                              active
                            >Plasmachain wallet is undergoing scheduled upgrades... Please check back in 1-2 hours.</b-tab>
                            <b-tab
                              title="Deposit"
                              v-if="userBalance.mainnetBalance > 0 && oracleEnabled"
                              active
                            >
                              <TransferStepper
                              ...
                              >
                                <template #pendingMessage>
                                  <p>Please approve deposit on your wallet...</p>
                                </template>
                                <template #failueMessage>
                                  <p>Approval failed.</p>
                                </template>
                                <template
                                  #confirmingMessage
                                >Please confirm deposit on your wallet. Depositing as soon as approval is confirmed:</template>
                              </TransferStepper>
                            </b-tab>

                            <b-tab
                              title="Withdraw"
                              v-if="oracleEnabled && (userBalance.loomBalance > 0 || unclaimWithdrawTokensETH > 0 || unclaimDepositTokens > 0)"
                            >
                              <template slot="title">
                                <span class="tab-title">Withdraw</span>
                                <fa
                                  icon="info-circle"
                                  v-if="unclaimWithdrawTokensETH > 0 || unclaimDepositTokens > 0"
                                  class="tab-icon text-red"
                                />
                              </template>
                              ...
                              <div v-if="unclaimWithdrawTokensETH > 0 && !gatewayBusy">
                                ...
                                <div class="center-children">
                                  <b-btn
                                    ...
                                    :disabled="isWithdrawalInprogress"
...
@Component({
  ...
  computed: {
    ...
    ...DappChainStore.mapState(["LoomTokenInstance", "dAppChainClient"]),
  ...
  methods: {
    ...
    ...DPOSStore.mapActions(["checkIfConnected"]),
    ...
    ...DappChainStore.mapMutations(["setWithdrewSignature"]),
...
export default class MyAccount extends Vue {
  ...
  errors = {
    onClickWithNoMapping:
      'No mapping detected, please click "Map Accounts" or refresh the page'
  };
  ...
  async checkUnclaimedLoomTokens() {
    ...
    if (this.unclaimDepositTokens > 0)
  ...
  async afterWithdrawalFailed() {
    await this.checkPendingWithdrawalReceipt();
    if (this.receipt) {
      this.unclaimWithdrawTokensETH = this.web3.utils.fromWei(
        this.receipt.amount.toString()
      );
      this.unclaimSignature = this.receipt.signature;
    }
    await this.refresh(true);
  ...
  async hasReceiptHandler(receipt) {
    if (receipt.signature && receipt.signature != this.withdrewSignature) {
      ...
      this.setErrorMsg(
        "Waiting for withdrawal authorization.  Please check back later."
      );
    }
  ...
  async mismatchedReceiptHandler(receipt, ethAddr) {
    ...
    let r = confirm(
      `A pending withdraw requires you to switch ETH accounts to: ${
        receipt.tokenOwner
      }. Please change your account and then click OK`
    );
    if (r) {
  ...
  async reclaimWithdrawHandler() {
    ...
    try {
      ...
      let tx = await this.withdrawCoinGatewayAsync({
      ...
      if (window.resolvingMismatchedReceipt) {
        alert(
          "Pending withdraw is fixed. Please log in again to switch back to the correct account."
        );
  ...
  async checkAllowance() {
    ...
    try {
      const allowance = await user.ethereumLoom.allowance(
        this.currentMetamaskAddress,
        gateway.address
      );
  ...
  async executeDeposit(amount, approvalTx) {
    ...
    let result = await this.dposUser._ethereumGateway.functions.depositERC20(
      weiAmount.toString(),
      this.dposUser.ethereumLoom.address
    );
