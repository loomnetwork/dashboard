 ...
 * @param tokenAmount
 */
export function withdrawToGateway(
  context: ActionContext,
  payload: { symbol: string; tokenAmount?: BN },
) {
  return timer(2000).toPromise()
...
export function checkUnclainedTokens(context: ActionContext) {
    return timer(2000).toPromise().then(() => {
        // context.state.unclailmedTokens = new BN("0")
    })
}
