...
declare type ActionContext = BareActionContext<DPOSState, DashboardState>
import { DashboardState, DposState } from '@/types';

async function initializeDependencies({ state, commit, dispatch }, payload) {
  commit("setShowLoadingSpinner", true);
  try {
    await dispatch("initWeb3Local");
    await dispatch("DappChain/ensureIdentityMappingExists", null, {
      root: true
    });
...
async function checkMappingAccountStatus({ state, commit, dispatch }) {
...
async function storePrivateKeyFromSeed({ commit }, payload) {
...
async function clearPrivateKey({ commit }, payload) {
  sessionStorage.removeItem("privatekey");
  commit("setIsLoggedIn", false);
}
async function checkIfConnected({ state, dispatch }) {
  if (!state.web3) await dispatch("initWeb3");
}
// broken
async function initWeb3Local({ commit, state, dispatch }) {
  if (state.walletType === "metamask") {
    let web3js = await web3init();
...
async function initWeb3({ rootState, dispatch, commit }) {
  let web3js;
  // @ts-ignore
  if (window.ethereum) {
    // @ts-ignore
 ...
 * @param {*} param0
 */
async function getValidatorList({ dispatch, commit, state }) {
  try {
...
async function checkAllDelegations({ state, rootState, commit }) {
  console.assert(
    !!rootState.DappChain.dposUser,
...
async function consolidateDelegations({ rootState }, validator) {
  console.assert(
    !!rootState.DappChain.dposUser,
    "expected dposUser to be initialised"
  );
...
async function queryRewards({ rootState, commit }) {
...
async function getTimeUntilElectionsAsync({ commit, dispatch }) {
...
async function redelegateAsync({ rootState, commit }, payload) {
  if (!rootState.DappChain.dposUser) {
    throw new Error("Expected dposUser to be initialized");
  }
...
async function fetchDappChainEvents({ state, commit }) {
  let historyPromise = axios.get(
    `${state.dappChainEventUrl}/eth:${
      state.currentMetamaskAddress
    }?sort=-block_height`
  );
...
async function fetchAnalyticsData({ state, commit, dispatch }, payload) {
  ...
  let dataPromise = await axios.get(
    url + "/delegation/total?from_date&to_date"
  );
  commit("setAnalyticsData", dataPromise);
}
async function loadEthereumHistory({ commit, rootState, getters, state }) {
...
async function updateDailyWithdrawLimit({ state }, history) {
 ...
 * @see dposPlugin
 */
async function approveDeposit({ rootState, commit }, tokenAmount) {
  console.assert(
    rootState.DappChain.dposUser,
  ...
  return executeTx(commit, "deposit approval", () =>
 ...
 */
async function executeDeposit({ rootState, commit }) {
  console.assert(
    rootState.DappChain.dposUser,
    "Expected dposUser to be initialized"
  );
  ...
  await executeTx(commit, "deposit", () =>
