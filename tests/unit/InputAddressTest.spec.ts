...
describe("InputAddress: Invalid address cases", () => {
  ...
  it("Should show an error message when bnb address is invalid", () => {
    const wrapper = factory({ value: "", chain: BNB_ADDRESS })
    // @ts-ignore
    wrapper.vm.onInput("0Xa777afeF41c00b2570A9ac3878F5D92e838f3371")
    expect(wrapper.find(".error").text()).to.equal(`Invalid ${BNB_ADDRESS} address.`)
  })

  it("Should show an error message when bnb address has more than 44 char", () => {
    const wrapper = factory({ value: "", chain: BNB_ADDRESS })
    // @ts-ignore
  ...
  it("Should show an error message when bnb address has less than 44 char", () => {
    const wrapper = factory({ value: "", chain: BNB_ADDRESS })
    // @ts-ignore
