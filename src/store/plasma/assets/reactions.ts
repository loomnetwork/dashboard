...
export function zbcardsReactions(store: Store<HasPlasmaState>) {
    ...
    store.watch(
        ...
        (web3) => {
            ...
            // @ts-ignore
            const cardInstance = new Contract(
                MigratedZBGCardJSON.abi,
                MigratedZBGCardJSON.networks[envName].address) as MigratedZBGCard
            plasmaModule.setCardContract(cardInstance)

            for (const pack of PACKS_NAME) {
                const packInstance = new Contract(BoosterPackJSON.abi,
